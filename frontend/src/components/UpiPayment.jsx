// UpiPayment.jsx — After payment success, marks loan as funded in MongoDB
...
async function getLiveRates() {
  ...
  try {
    ...
    const fb = {}; CURRENCIES.forEach(c=>{fb[c.code.toLowerCase()]=c.fb;}); return fb;
  }
}

/* ─── Payment Modal ───────────────────────────────────── */
function PaymentModal({ inrAmount, ethAmount, purpose, loanId, lenderAddress, onSuccess, onClose }) {
  ...
  const verifyOtp = async () => {
    ...
    // ── CRITICAL: mark loan as funded in MongoDB ──────────
    try {
      const tok = localStorage.getItem('ef-token');
      if (loanId && lenderAddress) {
        await api.post(`/loans/${loanId}/fund`, {
          lenderAddress,
          lenderName:    'UPI Payment',
          amount:        ethAmount.toString(),
          amountFunded:  ethAmount.toString(),
          isFullyFunded: true,
          paymentMethod: 'upi_inr',
          inrAmount:     inrAmount,
        }, { headers:{ Authorization:`Bearer ${tok}` } });
      }
    } catch(e) { console.warn('Fund record error:', e.message); }

    setLoading(false);
    setStep(3);
    const payId = 'pay_' + Math.random().toString(36).slice(2,14).toUpperCase();
    setTimeout(() => {
      onSuccess?.({ paymentId:payId, inrAmount, ethAmount, method:tab });
      onClose?.();
    }, 2000);
  };
  ...
  return (
    <div onClick={e=>e.stopPropagation()} style={{ position:'fixed', inset:0, zIndex:9999, background:'rgba(0,0,0,0.7)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'#fff', borderRadius:'18px', width:'100%', maxWidth:'420px', overflow:'hidden', boxShadow:'0 24px 60px rgba(0,0,0,0.4)', maxHeight:'90vh', overflowY:'auto' }}>
        ...
        <div style={{ background:'linear-gradient(135deg,#00e87a,#00c965)', padding:'16px 20px', display:'flex', justifyContent:'space-between', alignItems:'center', position:'sticky', top:0, zIndex:1 }}>
          <div>
            ...
            <div style={{ color:'rgba(0,0,0,0.55)', fontSize:'12px' }}>Secure · Demo Mode · No real money</div>
          </div>
          <div style={{ textAlign:'right' }}>
            <div style={{ fontWeight:900, color:'#000', fontSize:'1.5rem', lineHeight:1 }}>₹{inrAmount.toLocaleString('en-IN')}</div>
            <div style={{ fontSize:'11px', color:'rgba(0,0,0,0.55)' }}>≈ {parseFloat(ethAmount).toFixed(4)} ETH</div>
          </div>
        </div>
            ...
            <>
              ...
              <div style={{ display:'flex', marginBottom:'16px', border:'1px solid #e5e7eb', borderRadius:'10px', overflow:'hidden' }}>
                {[['card','💳 Card'],['upi','📱 UPI'],['netbanking','🏦 Netbanking']].map(([t,l])=>(
                  <button key={t} onClick={()=>{setTab(t);setError('');}}
                    style={{ flex:1, padding:'9px 0', fontSize:'11px', fontWeight:700, border:'none', cursor:'pointer', background:tab===t?'#00e87a':'#f9fafb', color:tab===t?'#000':'#555', transition:'all 0.2s' }}>
                    {l}
                  </button>
                ))}
              </div>
              ...
              {tab==='card' && (
                <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
                  {[
                    { lbl:'Card Number', key:'number', ph:'4111 1111 1111 1111', fn:e=>setCard(p=>({...p,number:fmtCard(e.target.value)})), val:card.number, mono:true, max:19 },
                    { lbl:'Cardholder Name', key:'name', ph:'Name on card', fn:e=>setCard(p=>({...p,name:e.target.value})), val:card.name, mono:false },
                  ].map(f=>(
                    <div key={f.key}>
                      <label style={{ fontSize:'11px', fontWeight:700, color:'#374151', display:'block', marginBottom:'4px', textTransform:'uppercase', letterSpacing:'0.05em' }}>{f.lbl}</label>
                      <input value={f.val} onChange={f.fn} placeholder={f.ph} maxLength={f.max}
                        style={{ width:'100%', padding:'9px 12px', borderRadius:'8px', border:'1.5px solid #d1d5db', fontSize:'14px', fontFamily:f.mono?'monospace':'inherit', outline:'none', boxSizing:'border-box', transition:'border-color 0.2s' }}
                        onFocus={e=>e.target.style.borderColor='#00c965'} onBlur={e=>e.target.style.borderColor='#d1d5db'} />
                    </div>
                  ))}
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
                    <div>
                      <label style={{ fontSize:'11px', fontWeight:700, color:'#374151', display:'block', marginBottom:'4px', textTransform:'uppercase', letterSpacing:'0.05em' }}>Expiry</label>
                      <input value={card.expiry} onChange={e=>setCard(p=>({...p,expiry:fmtExp(e.target.value)}))} placeholder="12/28" maxLength={5}
                  ...
                  <div style={{ padding:'8px 12px', background:'#f0fff8', borderRadius:'8px', fontSize:'11px', color:'#065f46', lineHeight:1.7 }}>
                    💡 Test: <strong>4111 1111 1111 1111</strong> · Exp: <strong>12/28</strong> · CVV: <strong>123</strong>
                  </div>
                </div>
              )}
              ...
              {tab==='upi' && (
                <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
                  <div>
                    <label style={{ fontSize:'11px', fontWeight:700, color:'#374151', display:'block', marginBottom:'4px', textTransform:'uppercase', letterSpacing:'0.05em' }}>UPI ID</label>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'8px' }}>
                    {['GPay','PhonePe','Paytm','BHIM','Amazon Pay','WhatsApp Pay'].map(app=>(
                      <button key={app} onClick={()=>setUpiId(app.toLowerCase().replace(/\s/g,'')+'@ok')}
                        style={{ padding:'8px 4px', borderRadius:'8px', border:'1.5px solid #e5e7eb', background:'#f9fafb', fontSize:'11px', fontWeight:700, cursor:'pointer', color:'#374151' }}>
                        {app}
                      </button>
                    ))}
                  </div>
                  <div style={{ padding:'8px 12px', background:'#f0fff8', borderRadius:'8px', fontSize:'11px', color:'#065f46' }}>
                    💡 Test UPI ID: <strong>success@upi</strong>
                  </div>
                </div>
              )}
              ...
              {tab==='netbanking' && (
                <div>
                  <label style={{ fontSize:'11px', fontWeight:700, color:'#374151', display:'block', marginBottom:'10px', textTransform:'uppercase', letterSpacing:'0.05em' }}>Select Your Bank</label>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px' }}>
                    {banks.map(b=>(
                      <button key={b} onClick={()=>setBank(b)}
                        style={{ padding:'9px 10px', borderRadius:'8px', border:`1.5px solid ${bank===b?'#00c965':'#e5e7eb'}`, background:bank===b?'#f0fff8':'#f9fafb', fontSize:'12px', fontWeight:bank===b?700:500, cursor:'pointer', color:bank===b?'#065f46':'#374151', textAlign:'left', transition:'all 0.15s' }}>
                        {b}
                      </button>
                    ))}
                  </div>
              ...
              <div style={{ display:'flex', gap:'8px', marginTop:'16px' }}>
                ...
                <button onClick={submit} disabled={loading}
                  style={{ flex:2, padding:'10px', borderRadius:'10px', border:'none', background:loading?'rgba(0,200,100,0.35)':'#00e87a', color:'#000', fontWeight:800, cursor:loading?'not-allowed':'pointer', fontSize:'13px' }}>
                  {loading?'⏳ Processing...`':'Pay ₹'+inrAmount.toLocaleString('en-IN')}
                </button>
              </div>
            </>
          )}
          ...
          {step===2 && (
            <div style={{ textAlign:'center' }}>
              ...
              <p style={{ fontSize:'13px', color:'#6b7280', marginBottom:'20px', lineHeight:1.6 }}>
                Enter any 6 digits for demo.<br/><strong style={{color:'#065f46'}}>No real OTP is sent.</strong>
              </p>
          ...
          {step===3 && (
            <div style={{ textAlign:'center', padding:'8px 0' }}>
              ...
              <p style={{ fontSize:'13px', color:'#6b7280', marginBottom:'16px', lineHeight:1.7 }}>
                ₹{inrAmount.toLocaleString('en-IN')} paid.<br/>
                The loan has been marked as funded.<br/>
                <span style={{color:'#9ca3af',fontSize:'12px'}}>Demo — no real money charged.</span>
              </p>
              <div style={{ padding:'12px 16px', background:'#f0fff8', borderRadius:'10px', border:'1px solid #a7f3d0', fontSize:'12px', color:'#065f46', textAlign:'left', lineHeight:2 }}>
                <div>Amount paid: <strong>₹{inrAmount.toLocaleString('en-IN')}</strong></div>
                <div>ETH equivalent: <strong>{parseFloat(ethAmount).toFixed(4)} ETH</strong></div>
                <div>Loan #{loanId}: <strong style={{color:'#00c965'}}>Funded ✓</strong></div>
              </div>
            </div>
          )}

          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'5px', marginTop:'14px', fontSize:'11px', color:'#9ca3af' }}>
            🔒 EqualFund Payments · Demo Mode
          </div>
        </div>
      </div>
    </div>
...
/* ─── UPI Pay Button ──────────────────────────────────── */
export function UpiPayButton({ ethAmount, purpose, loanId, lenderAddress, onSuccess }) {
        ...
        {open && (
          <div style={{position:'absolute',top:'100%',right:0,zIndex:400,background:'var(--card-bg)',border:'1px solid var(--border)',borderRadius:'10px',padding:'5px',minWidth:'190px',boxShadow:'0 8px 24px rgba(0,0,0,0.15)',marginTop:'4px'}}>
            {CURRENCIES.map(c=>{
              ...
              return <button key={c.code} onClick={()=>{setCurrency(c.code);setOpen(false);}} style={{width:'100%',padding:'5px 8px',borderRadius:'7px',border:'none',background:currency===c.code?'var(--surface-3)':'transparent',cursor:'pointer',textAlign:'left',display:'flex',justifyContent:'space-between',fontSize:'12px',color:'var(--ink)'}}>
