        ...
        <div style={{ marginBottom:'1.5rem', display:'flex', flexDirection:'column', gap:'10px' }}>
          ...
          {showFilters && (
            <div style={{ padding:'1rem', background:'var(--surface-3)', borderRadius:'12px', border:'1px solid var(--border)', display:'flex', gap:'1rem', flexWrap:'wrap', alignItems:'flex-end' }}>
              ...
              <button onClick={() => { setCategory('all'); setMinRate(''); setMaxRate(''); setSearch(''); setSortBy('newest'); }} className="btn btn-sm" style={{ background:'rgba(239,68,68,0.1)', color:'#dc2626', border:'1px solid rgba(239,68,68,0.2)', borderRadius:'8px', padding:'6px 12px', cursor:'pointer', fontSize:'12px', fontWeight:700 }}>
                Reset
              </button>
            </div>
          )}
        ...
        {fetchingLoans ? (
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(290px,1fr))', gap:'1rem' }}>
            {filtered.map(loan => {
              ...
              return (
                <div key={loan.id} className="card card-hover" style={{ padding:'1.5rem', cursor:'pointer' }}>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', marginBottom:'12px' }}>
                    {[
                      ...
                      <div key={l}>
                        ...
                        <div style={{ fontWeight:900, color:c, fontSize:'15px', letterSpacing:'-0.02em', fontFamily:v.includes('ETH')||v.includes('%')?'monospace':'inherit' }}>{v}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'12px', paddingBottom:'12px', borderBottom:'1px solid var(--border)' }}>
                    <div style={{ width:'24px', height:'24px', borderRadius:'6px', background:'var(--mint-pale)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'11px', fontWeight:800, color:'var(--mint-dim)' }}>
